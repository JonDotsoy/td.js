...
var transformEjsCollection = function (files, preRender, data, cb) {
	async.map(files, function (file, cbByMap) {
		...
		cb(err, f.filter( function ( e ) { return e !== null; } ));
	});
}
...
var writeFileByCollect = function (files, preWrite, cb) {
	async.map(
		...
		function (err, filesWrites) {
			cb(err, filesWrites.map( function (e) { return e !== null; } ) );
		}
	);
}
 ...
 * @param      {string}  [params.dir=process.cwd()]  Project directory.
...
var InitializeAProject = function (params) {
	...
	while (params.args.length !== 0) {
		var index = params.args.shift();
		var e;
		if (e = index.match(/^\-{2}([^=]+)$/)) {
			dataParameters.parameters[e[1]] = params.args.shift() || true;
		} else
		if (e = index.match(/^\-{2}([^=]+)=(.+)$/)) {
		...
		.series([
					...
					function ( err, statFile ) {
						if (err) { cb(); }
							...
							tdInitializeControl.initialize(
								...
								function (name, value) {
									switch (name) {
										case 'ignore': pathsIgnore.push(value); break;
										case 'parameters':
											if (_.isObject(value)) {
			...
			function (cb) {
				transformEjsCollection (
					...
					{
						package: _.get.bind(_, params.pkg),
						_: _,
						async: async,
